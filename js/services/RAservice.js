...
//			},
			...
			searchresource:function(skills,jobcategory,location,yearsOfExperience){
				...
				$http.get('rest/resource/findResourcesByFilter/'+regid+'/'+skills+'/'+jobcategory+'/'+location+'/'+yearsOfExperience+'/1/10').success(function(response){
					deferred.resolve(response);
				}).error(function(err){
					deferred.reject(err);
				})
			...
			searchrequirement:function(skills,jobcategory,location,experience){
				...
				var deferred = $q.defer();
				var regid = localStorage.getItem('registrationId');
				$http.get('rest/requirement/findRequirementsByFilter/'+regid+'/'+skills+'/'+jobcategory+'/'+location+'/'+experience+'/1/10').success(function(response){
					deferred.resolve(response);
				}).error(function(err){
					deferred.reject(err);
				})
